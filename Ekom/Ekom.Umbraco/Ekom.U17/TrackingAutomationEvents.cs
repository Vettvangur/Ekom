...
internal sealed class TrackingAutomationEvents : IComponent
{
    ...
    private async Task OnAddedOrderlineMetaAsync(object sender, AddedOrderlineEventArgs args, CancellationToken cancellationToken)
    {
    ...
    private async Task OnCustomerEmailAddedMetaAsync(object sender, CustomerEmailAddedEventArgs args, CancellationToken cancellationToken)
    {
    ...
    private async Task OnCompleteCheckoutAsync(object sender, CompleteCheckoutEventArgs args, CancellationToken cancellationToken)
    {
        ...
        if (_options.Ga4.Enabled)
        {
            ...
            await TrackingEvents.OnGa4PurchasePreparingAsync(this, eventArgs, cancellationToken).ConfigureAwait(false);

            if (!eventArgs.Cancel)
            {
                var dispatcher = scope.ServiceProvider.GetRequiredService<IGa4TrackingDispatcher>();
                await dispatcher.EnqueueAsync(eventArgs.Request, cancellationToken).ConfigureAwait(false);
            }
        }
        ...
        if (_options.Meta.Enabled)
        {
            ...
            await TrackingEvents.OnMetaPurchasePreparingAsync(this, eventArgs, cancellationToken).ConfigureAwait(false);
