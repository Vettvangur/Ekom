    ...
    public void Create()
    {
        ...
        if (!dbSchema.Tables.Any(t => t.TableName == "EkomPaymentOrders"))
        {
            ...
            db.Execute(@"ALTER TABLE EkomPaymentOrders ADD
                PaymentProviderName  AS json_value([NetPaymentSettingsData],'$.PaymentProviderName.saldkfjaskldf.asdfasdf.asdf')");
            db.Execute($"ALTER TABLE EkomPaymentOrders ALTER COLUMN NetPaymentSettingsData NVARCHAR(MAX)");
