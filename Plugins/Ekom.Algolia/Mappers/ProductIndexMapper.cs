...
internal sealed class ProductIndexMapper : IAlgoliaProductIndexMapper
{
    ...
    public AlgoliaProductRecord? Map(IProduct product, AlgoliaResolvedStore store, string baseIndexName)
    {
        ...
        var record = new AlgoliaProductRecord
        {
            ...
            PriceFormatted = NullIfWhiteSpace(price?.WithVat.CurrencyString),
            PriceWithVatFormatted = NullIfWhiteSpace(price?.WithVat.CurrencyString),
            PriceWithoutVatFormatted = NullIfWhiteSpace(price?.WithoutVat.CurrencyString),
            Discounted = price?.HasDiscount ?? false,
            OriginalPrice = price?.BeforeDiscount.Value,
            OriginalPriceFormatted = NullIfWhiteSpace(price?.BeforeDiscount.CurrencyString),
        ...
        IReadOnlyList<ConfiguredVariantFacet> variantFacetAttributes)
    {
        ...
        return new AlgoliaProductRecord
        {
            ...
            PriceFormatted = NullIfWhiteSpace(price?.WithVat.CurrencyString) ?? productRecord.PriceFormatted,
            PriceWithVatFormatted = NullIfWhiteSpace(price?.WithVat.CurrencyString) ?? productRecord.PriceWithVatFormatted,
            PriceWithoutVatFormatted = NullIfWhiteSpace(price?.WithoutVat.CurrencyString) ?? productRecord.PriceWithoutVatFormatted,
            Discounted = price?.HasDiscount ?? productRecord.Discounted,
            OriginalPrice = price?.BeforeDiscount.Value ?? productRecord.OriginalPrice,
            OriginalPriceFormatted = NullIfWhiteSpace(price?.BeforeDiscount.CurrencyString) ?? productRecord.OriginalPriceFormatted,
