...
export default class OrdersContainer extends Component {
  ...
  toggleRowDelete(UniqueId) {
    const { deleteRows } = this.state;
    if (!deleteRows[UniqueId]) {
      const newSelected = Object.assign({}, deleteRows);
      newSelected[UniqueId] = !deleteRows[UniqueId];
      this.setState({
        deleteRows: newSelected,
      });
    } else {
  ...
  toggleSelectAll() {
    const { selectAll, orders } = this.state;
    const newSelected = {};
    ...
    const columns = [
      {
        ...
        Cell: ({ original }) => (
          <input
            type="checkbox"
            className="checkbox"
            checked={selected[original.UniqueId] === true}
        ...
        <ReactTable
          ...
          getTrProps={(rowInfo) => {
            if (rowInfo && rowInfo.original) {
              if (selected[rowInfo.original.UniqueId]) {
                return {
                  style: {
              ...
              if (deleteRows[rowInfo.original.UniqueId]) {
                return {
                  style: {
